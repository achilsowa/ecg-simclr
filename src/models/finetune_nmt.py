...
from src.models.simclr import simclr_ecgresnet, EcgResnet
...
class NMT(nn.Module):
    """ Simple Neural ECG Translation Model:
        - Bidrectional LSTM Encoder [Will be replaced later by a key value attention without positioning embedding]
        - Unidirection LSTM Decoder
        - Global Attention Model (Luong, et al. 2015)
    """

    def __init__(self, embed_size, hidden_size, vocab, input_size=625, dropout_rate=0.2):
        """ Init NMT Model.
        ...
        self.target_embedding = ModelEmbeddings(embed_size, vocab).target
        ...
        # default values
        self.pre_embed_ecg = None
        self.post_embed_ecg = None
        self.encoder = None
        self.decoder = None
        ...
        self.encoder = nn.LSTM(input_size=self.input_size, hidden_size=hidden_size, num_layers=2, bidirectional=True)
    ...
    def forward(self, source: torch.Tensor, target: List[List[str]]) -> torch.Tensor:
        """ Take a mini-batch of source and target sentences, compute the log-likelihood of
        target sentences under the language models learned by the NMT system.

        @param source (torch.FloatTensor of shape (B, num_ecg_embed, ecg_embed_size)): list of source ecgs
    ...
    def encode(self, source_ecgs: torch.Tensor, source_lengths: List[int]) -> Tuple[
        torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """ Apply the encoder to source sentences to obtain encoder hidden states.
            Additionally, take the final states of the encoder and project them to obtain initial states for decoder.

        @param source_ecgs (Tensor): Tensor of source ecgs with shape (b, 5, 512), where
                                        b = batch_size.
        ...
        # X = self.ecg_embed(source_ecgs).permute(1, 0, 2)
        X = source_ecgs.permute(1, 0, 2)

        # The result of the following is to remove padding before passing the data to the RNN. While removing we keep information
        # each seq_length. The point is guess, is to not apply the RNN step when we are done with the corresponding sentence.
        # Technically, we could still apply it and rely on target to zero out the gradient w.r.t those step, but it will be more
        # efficient to not apply it in the first place
        X = pack_padded_sequence(X, source_lengths, enforce_sorted=False) 
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)

        # We then reverse pack_padded_sequence to get back results in the classic shape they would have been returned
        # if we had not used pack_padded_sequence in the inputs. In our case it would have been (src_len, b, 2*h)
        # So we permute to have it in the shape (b, src_len, 2*h)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=-1))
        init_dedocer_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim=-1))
        dec_init_state = (init_decoder_hidden, init_dedocer_cell)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.target_embedding(target_padded)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1)).squeeze(dim=-1)
    ...
    def beam_search(self, src_ecg: torch.Tensor, beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        Hypothesis]:
        """ Given a single source sentence, perform beam search, yielding translations in the target language.
        @param src_ecg (torch.Tensor): a single ecg
        ...
        src_ecgs = src_ecg.unsqueeze(0)

        src_encodings, dec_init_vec = self.encode(src_ecgs, [len(src_ecg)])
    ...
    def device(self) -> torch.device:
        ...
        """
        return self.target_embedding.weight.device
    ...
    def save(self, path: str):
        ...
        params = {
            'args': dict(embed_size=self.target_embedding.embed_size, hidden_size=self.hidden_size,
...
def simclr_finetuned_nmt(vocab_path):
    encoder = EcgResnet(
        in_length=2500, 
        in_channels=12, 
        n_grps=4, 
